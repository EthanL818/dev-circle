...
export async function fetchPostsByTech(techName) {
  ...
  try {
    ...
    const posts = querySnapshot.docs.map(postToJSON); // Assuming postToJSON function is defined correctly
    return posts;
  } catch (error) {
    console.error("Error fetching posts by tech:", error);
 ...
 */
export function extractImageUrlsFromContent(markdownContent) {
